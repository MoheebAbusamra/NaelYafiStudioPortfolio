...
/**
 * Hero illustration. The source is a JPEG, so it carries no alpha and its backdrop
 * is solid black, which would render as a black slab over the navy page. The build
 * keys that backdrop out to transparency; see `keyBlackBackground`.
 */
const HERO_CUTOUT = { from: "NewHouseHome.jpeg", to: "brand/home-house.webp" };
...
async function main() {
  ...
  // Hero cut out: key the black studio backdrop to alpha, then emit WebP.
  const heroSrc = path.join(ASSETS, HERO_CUTOUT.from);
  if (existsSync(heroSrc)) {
    const dest = path.join(PUBLIC, HERO_CUTOUT.to);
    await mkdir(path.dirname(dest), { recursive: true });

    const keyed = await keyBlackBackground(heroSrc);
    const info = await sharp(keyed)
      .resize({ width: 1800, withoutEnlargement: true })
      .webp({ quality: 88, alphaQuality: 90, effort: 5 })
      .toFile(dest);

    console.log(`[assets] hero cut out ${info.width}x${info.height}  ${(info.size / 1024).toFixed(0)} KB`);
  } else {
    console.warn(`[assets] Missing hero image: Assets/${HERO_CUTOUT.from}`);
  }
